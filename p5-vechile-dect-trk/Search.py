...
def  test() :
    ...
    images = sorted(glob.glob(TEST_IMAGES_DIR + '*.jpg'))
    # images
...
if __name__ == '__main__':
    ...
    data_file = DATA_DIR + 'ProcessedData.p'
    ...
    data_file = DATA_DIR + 'ClassifierData.p'
    with open(data_file, mode='rb') as f:
        data = pickle.load(f)
    ...
    outfile=OUTPUT_IMAGES+'processed_'+inpfile
...
'''
    ...
    plt.savefig(IMAGES_DIR + 'heatmap.png')

    plt.figure()
    labels = label(final_heatmap)
    print(labels[1], 'cars found')
    plt.imshow(labels[0], cmap='gray')
    plt.savefig(IMAGES_DIR + 'labels.png')
    ...
    plt.imshow(draw_img)
    #plt.show()
    plt.savefig(IMAGES_DIR + 'bounding_boxes.png')
'''
